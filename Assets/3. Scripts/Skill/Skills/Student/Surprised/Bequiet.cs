...
[Serializable]
public class Bequiet : Skill, ITargetedEnemySkill, IAttackSkill
{
    ...
    public override IEnumerator Effected(SkillContext skillContext)
    {
        ...
        {
            foreach(Character chat in skillContext.targets)
            {
                chat.AddIcon(SkillTemplet.FindIcon(data.skillIcons, typeof(NoRun)), 4);
            }
        }
